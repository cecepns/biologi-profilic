...
import { Award, GraduationCap, CheckCircle2, BookOpen, Star, Sparkles, ShieldCheck } from 'lucide-react';
      ...
      <div className="bg-white rounded-3xl border border-slate-150 p-6 sm:p-8 shadow-sm flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-left">
        ...
        <div className="flex-1 space-y-2">
          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
            <h1 className="text-xl sm:text-2xl font-black text-slate-900">{user?.name || 'Ahmad Fauzan'}</h1>
          ...
          <p className="text-xs sm:text-sm text-slate-500 font-medium">
            NIS: <strong>{user?.nis || '20261101'}</strong> • Kelas: <strong>{user?.className || 'XI IPA 2'}</strong> • Kelompok 1 (Fitoplankton)
          </p>
        ...
        <div className="p-4 bg-emerald-600 text-white rounded-3xl text-center shadow-md shadow-emerald-600/20 min-w-[130px]">
          ...
          <span className="text-[10px] bg-white/20 px-2 py-0.5 rounded-full block mt-1 font-bold">Predikat: A</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-150 p-6 sm:p-8 shadow-sm space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-400">Rincian perolehan skor aktivitas individu dan kolaborasi kelompok.</p>
          </div>
