...
import { Presentation, MessageSquare, Award, Star, ExternalLink, Send, CheckCircle2, UploadCloud, HelpCircle, Loader2 } from 'lucide-react';
...
export const Stage4PresentationDiscussion = ({ stage, onComplete }) => {
  ...
  useEffect(() => {
    const fetchPresentationData = async () => {
      setLoading(true);
      try {
        const presRes = await request.get(API_ENDPOINTS.PRESENTATIONS.LIST, { stageId: 4 });
  ...
  const handleAddFeedback = async (e) => {
    e.preventDefault();
    if (!newComment.trim() || !presentation) return;

    setIsSubmittingComment(true);
    try {
      const res = await request.post(API_ENDPOINTS.PRESENTATIONS.FEEDBACKS(presentation.id || 1), {
        user_id: user?.id || 3,
      ...
      <div className="bg-white rounded-3xl border border-slate-150 p-6 sm:p-8 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">Jadwal Sesi: {presentation.date}</p>
          </div>

          <a
            href={presentation.slideUrl}
            target="_blank"
            rel="noreferrer"
        ...
        <div className="p-6 bg-slate-900 rounded-2xl text-white flex flex-col items-center justify-center text-center gap-3 min-h-[220px]">
          ...
          <div>
            <h4 className="font-extrabold text-sm sm:text-base">{presentation.title}</h4>
            <p className="text-xs text-slate-400 mt-1 max-w-md">{presentation.notes}</p>
          </div>
        ...
        {presentation.rubricScore && (
          <div className="p-6 rounded-2xl bg-purple-50/60 border border-purple-200/80 space-y-4">
            ...
            <div className="p-3 bg-white rounded-xl border border-purple-100 text-xs text-slate-700">
              <strong>Catatan Guru Maya:</strong> "{presentation.rubricScore.feedback}"
            </div>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-150 p-6 sm:p-8 shadow-sm space-y-6">
        ...
        <form onSubmit={handleAddFeedback} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              type="submit"
              className="px-5 py-2.5 bg-purple-600 hover:bg-purple-700 text-white rounded-xl font-bold text-xs sm:text-sm shadow-sm transition-colors flex items-center gap-1.5"
            >
              <Send size={15} />
              <span>Kirim</span>
            </button>
